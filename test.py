                ...
                with open(file, "r", encoding="utf-8") as f:
                    for line in f:
                        ...
                        if line.startswith('  Vessel:'):                       
                            self.ship, self.reis = re.findall(r'\w+|\w+ \w+', line[line.find(':', 8)+1:line.find('|', 22)]), \
                                re.findall(r'\w*', line[line.find(':', 32)+1:line.find('|', 48)])
                            # print(len(self.ship))
...
# 2. Номер станции и оператор
                            ...
                            self.station, self.operator = re.findall(r'\w*', line[line.find(':', 8)+1:line.find('|', 22)]), \
                                re.findall(r'\w*', line[line.find(':', 32)+1:line.find('|', 48)])
...
# скип
 ...
 # 5. Время и давление
                            ...
                            self.st_time, self.press = re.findall(r'\d{1,2}:\d{2}:', line[line.find(':', 8)+1:line.find('|', 22)]), \
                                re.findall(r'\d*\.?\d+', line[line.find(':', 32)+1:line.find('|', 48)])
...
# 6. Широта и температура воздуха
                            ...
                            self.st_lat, self.t_air = re.findall(r'-?\d{1,3}(?::\d{2}){1,2}(?:\.\d{1,3})?', line[line.find(':', 8)+1:line.find('|', 22)]),\
                                re.findall(r'-?\d*\.?\d+', line[line.find(':', 32)+1:line.find('|', 48)])
...
# 7. Долгота и ветер (градус и скорость)
                            ...
                            self.st_lon, self.wind = re.findall(r'-?\d{1,3}(?::\d{2}){1,2}(?:\.\d{1,3})?', line[line.find(':', 8)+1:line.find('|', 22)]),\
                                re.findall(r'(-?\d*\.?\d+)*/(-?\d*\.?\d+)*', line[line.find(':', 32)+1:line.find('|', 48)])
...
# 8. Глубина и параметры волны
                            ...
                            self.depth, self.wave = re.findall(r'\w*', line[line.find(':', 8)+1:line.find('|', 22)]),\
                                re.findall(r'(-?\d*\.?\d+)*/(-?\d*\.?\d+)*', line[line.find(':', 32)+1:line.find('|', 48)])
...
# 9. Облачность
                            ...
                            self.cloud = re.findall(r'(-?\d*\.?\d+)*/(-?\d*\.?\d+)*/?([a-zA-Z]+)*\s?/?([a-zA-Z]+)*',
                                                    line[line.find(':', 32)+1:line.find('|', 48)])
...
# Тут мы проходимся по полученному выше массиву с целью сделать словарь (структуру с полями, как в БД), которая будет использоваться как таблица далее.
        ...
        self.cursor = DatabaseUploader.connect(self,
        ...
        try:
            query_ship = 'SELECT radio_sign FROM ships_reestr WHERE ves_lat like %s;'
        ...
        self.df['st_time'] = self.df['st_time'].apply(lambda x: datetime.strptime(x.strip(), '%H:%M:').time())
# ----------------------------------------------------------------------------------------------------
        self.cursor_reg = DatabaseUploader.connect(self,
                                                    config['postgresql']['database'],
                                                    config['postgresql']['user'],
                                                    config['postgresql']['password'],
                                                    config['postgresql']['host'])
...
# ----------------------------------------------------------------------------------------------------
        self.rzr1_df = Rzr.define_rzr1(self, self.points)
        self.rzr2_df = Rzr.define_rzr2(self, self.points)

        self.df['rzr1'] = self.rzr1_df['rzr1']
        self.df['rzr2'] = self.rzr2_df['rzr2']
# ----------------------------------------------------------------------------------------------------
        ...
        self.download_dict = {'ship': list(self.df['ship']), 'reis': list(self.df['reis']), 'nst_g': list(self.df['nst_g']),
                              ...
                              'st_time': list(self.df['st_time']),'rnloc': list(self.df['rnloc']), 'rnzon': list(self.df['rnzon']),
                              'rnikes': list(self.df['rnikes']), 'shir_dgr': list(self.df['shir_dgr']), 'dolg_dgr': list(self.df['dolg_dgr']),
                              'shir_dsec': list(self.df['shir_dsec']), 'dolg_dsec': list(self.df['dolg_dsec']), 'glub': list(self.df['glub']),
                              'press': list(self.df['press']), 't_air': list(self.df['t_air']), 'v_wind': list(self.df['v_wind']),
                              'f_wind': list(self.df['f_wind']), 'v_wave': list(self.df['v_wave']), 'h_wave': list(self.df['h_wave']),
                              'visibil': list(self.df['visibil']), 'cloud': list(self.df['cloud']), 'clo_nyr': list(self.df['clo_nyr']),
                              'for_clo': list(self.df['for_clo']), 'tr_water': None, 'rzr1': list(self.df['rzr1']), 'rzr2': list(self.df['rzr2']),
                              'weather': list(self.df['weather']), 'operator': list(self.df['operator']), 'interpolated': is_interpolated}
        ...
        self.table_uploading_station.model.df = self.new_df
